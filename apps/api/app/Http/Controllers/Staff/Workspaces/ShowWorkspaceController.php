...
namespace App\Http\Controllers\Staff\Workspaces;
...
class ShowWorkspaceController extends Controller
{
    ...
    {
        Gate::authorize('accessStaffPortal', Workspace::class);

        /** @var User $user */
        $user = $request->user();

        $workspaceModel = $showWorkspace->handle($user, $workspace, Portal::rolesFor(Portal::STAFF));
