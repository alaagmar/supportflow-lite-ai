...
Your project is a portfolio-grade Laravel + Next.js AI SaaS with queues, policy retrieval, human review, AI run logs, and provider-agnostic fallback (Mistral primary + mock fallback). Docker must reflect that system shape: API, web, worker, scheduler, Postgres, Redis, reverse proxy, and dev-only email tooling. 
